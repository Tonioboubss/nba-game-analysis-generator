...
def main():
    ...
    parser.add_argument("--no-repeat-ngram-size", type=int, default=3,
                         help="Block repeating any n-gram of this size — beam search on an "
                              "undertrained checkpoint loops ('X - X - X - X...') without this.")
    parser.add_argument("--repetition-penalty", type=float, default=1.3,
                         help="Down-weight tokens already generated. >1.0 discourages repetition; "
                              "1.0 disables it.")
            ...
            with torch.no_grad():
                output_ids = model.generate(
                    **inputs, max_new_tokens=args.max_new_tokens,
                    num_beams=4,  # beam search here: this is for evaluation, not
